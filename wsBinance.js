export class WebSocketService {
    ...
    createWebSocket(symbol, callback) {
        ...
        fetchOrderbook(this.#lowercaseSymbol)
            .then(depth_snapshot => {
                this.#socket = new WebSocket(`wss://fstream.binance.com/stream?streams=${this.#lowercaseSymbol}@aggTrade/${this.#lowercaseSymbol}@depth@100ms/${this.#lowercaseSymbol}@kline_1m`); 
    ...
    async reinitializeOrderBook(symbol) {
        ...
        this.last_update_id = depth_snapshot.lastUpdateId;
        this.order_book.order_book = this.order_book.initialize_order_book(depth_snapshot.bids, depth_snapshot.asks);
        this.#aggTradeBuffer = [];
    }
}
...
async function fetchOrderbook(symbol) {
	const response = await fetch(`https://fapi.binance.com/fapi/v1/depth?symbol=${symbol}&limit=500`);
