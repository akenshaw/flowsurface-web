export async function combineDicts() {
  ...
  const symbols = Object.keys(fr_dict).filter(symbol => 
    !["BTCSTUSDT", "CVCUSDT", "DOGEUSDC", "COCOSUSDT", "BNBBTC", "ETHBTC", "RAYUSDT", "HNTUSDT", "SCUSDT", "WIFUSDT", "BTSUSDT", "TOMOUSDT", "FTTUSDT", "SRMUSDT"].includes(symbol) 
    && !symbol.includes('_')
  );  
  const hist_OI_promises = symbols.map(symbol => fetch_hist_OI(symbol, startTime, endTime));
  const hist_OI_data_arr = await Promise.all(hist_OI_promises);

  symbols.forEach((symbol, i) => {
    if (turnovers_dict.hasOwnProperty(symbol)) {
      combined_dict[symbol] = {...fr_dict[symbol], ...turnovers_dict[symbol], ...hist_OI_data_arr[i]};
    }
  });
...
async function fetch_hist_OI(symbol, startTime, endTime) {
  ...
  try {
    ...
    const response = await fetch(`https://fapi.binance.com/futures/data/openInterestHist?symbol=${symbol}&period=30m&limit=1&startTime=${startTime}&endTime=${endTime}`);
    const data = await response.json();

    const OI_24hrAgo = Math.round(data[0].sumOpenInterest);
    const OI_24hrChange = Math.round((current_OI - OI_24hrAgo) / OI_24hrAgo * 10000) / 100;
